  ...
  const fetchSubscriptions = async () => {
    ...
    try {
        ...
        const expiringSoonSubscriptions = response.data.filter(sub => {
            const remainingDays = calculateRemainingDays(sub.createdAt, sub.subscriptionType);
            return remainingDays !== "Expired" && remainingDays <= 3;
        });
